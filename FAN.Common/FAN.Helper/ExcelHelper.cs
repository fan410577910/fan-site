...
namespace FAN.Helper
{
    ...
    /// </summary>
    /// <remarks>
    /// [2014-01-17] Develop by SunLiang,YangHuanWen,MengXianMin,WangYunPeng.
    /// </remarks>
    public static class ExcelHelper
    {
        ...
        public static List<T> Import<T>(Stream stream, IEnumerable<ColumnOption> fieldMapping) where T : class, new()
        {
            ...
            foreach (ColumnOption filed in fieldMapping)
            {
                //modify by Yang 2016年11月1日 17:26:49 此处代码前后台都在调用PropertyName-->Title 
                cell = row.Cells.FirstOrDefault(p => p.StringCellValue.Trim() == filed.Title);
                if (cell == null)
                {
                    continue;
                }
                filed.Index = cell.ColumnIndex;
                PropertyInfo pi = properties.FirstOrDefault(p => p.Name == filed.PropertyName);
                if (pi == null)
                {
                    continue;
                }
            ...
            try
            {
                for (rowIndex = rowIndex + 1, len = sheet.LastRowNum + 1; rowIndex < len; rowIndex++)
                {
                    T t = new T();
                    row = sheet.GetRow(rowIndex);
                    foreach (ColumnOption field in fieldMapping)
                    {
                        value = null;
                        stringValue = null;
                            ...
                            )
                        {
                            ...
                            {
                                throw new Exception("导入数据失败:在第[{0}]行，'{1}'列中数据非法！".Formater(rowIndex + 1, field.Title));
                            }
                            else
                            {
                                continue;
        ...
        public static MemoryStream Export<T>(IEnumerable<T> list, IEnumerable<ColumnOption> fieldMapping, string title = null) where T : class
        {
            ...
            #region - 右击文件 属性信息 -
            {
                ...
                si.Author = "TideBuy YangHuanWen"; //填加xls文件作者信息
                si.ApplicationName = "TideBuy YangHuanWen"; //填加xls文件创建程序信息
                si.LastAuthor = "TideBuy YangHuanWen"; //填加xls文件最后保存者信息
                si.Comments = "TideBuy YangHuanWen"; //填加xls文件作者信息
            ...
            if (title != null)
            {
                ...
                CellRangeAddress range = new CellRangeAddress(rowIndex, rowIndex, 0, fieldMapping.Count() - 1);
            ...
            foreach (ColumnOption field in fieldMapping)
            {
                ...
                if (field.ImportValueValidate != null)
                {
                    ...
                    ColumnValidate valueValidate = field.ImportValueValidate(cellFormater);
        ...
        /// <returns></returns>
        public static MemoryStream Export<T>(StreamReader reader, IEnumerable<ColumnOption> fieldMapping, string title = null) where T : class
        {
            ...
            #region - 右击文件 属性信息 -
            {
                ...
                workbook.DocumentSummaryInformation = dsi;
                SummaryInformation si = PropertySetFactory.CreateSummaryInformation();
                si.Author = "TideBuy WangYunPeng"; //填加xls文件作者信息
                si.ApplicationName = "TideBuy WangYunPeng"; //填加xls文件创建程序信息
                si.LastAuthor = "TideBuy WangYunPeng"; //填加xls文件最后保存者信息
                si.Comments = "TideBuy WangYunPeng"; //填加xls文件作者信息
                si.Title = title ?? "标题信息"; //填加xls文件标题信息
                si.Subject = title ?? "主题信息";//填加文件主题信息
                si.CreateDateTime = DateTime.Now;
                si.EditTime = 10;
            ...
            {
                item = JsonHelper.ConvertStrToJson<T>(line);
                //index++;//从1开始
                rowIndex++;
                colIndex = 0;
                row = sheet.CreateRow(rowIndex);
        ...
        /// <param name="fieldMapping">字段映射字典。对于key存在于list属性中，将使用list集合中属性的值；不存在的将使用value填充.</param>
        /// <returns></returns>
        public static MemoryStream Export<T>(Hashtable hashList, IEnumerable<ColumnOption> fieldMapping, IEnumerable<Sheet> sheetList, int listCount, string title = null) where T : class
        {
            ...
            #region sheet
            foreach (Sheet sheetItem in sheetList)
            {
                rowIndex = -1;
        ...
        /// <param name="fieldMapping">字段映射字典。对于key存在于list属性中，将使用list集合中属性的值；不存在的将使用value填充.</param>
        /// <returns></returns>
        public static MemoryStream Export<T>(DataTable dt, IEnumerable<ColumnOption> fieldMapping, string title = null) where T : class
        {
