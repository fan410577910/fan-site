...
namespace FAN.Helper
{
    ...
    public static class StringHelper
    {
        ...
        /// <summary></summary>
        public const string SQL_PARAMETER = "?";

        public const int ALIAS_TRUNCATE_LENGTH = 10;
        ...
        public static string Replace(string template, string placeholder, string replacement, bool wholeWords)
        {
            ...
            else
            {
                ...
                if (loc + placeholder.Length < template.Length)
                {
                    ...
                    //After a token in HQL there can be WHITE_SPACE, CLOSED_PAREN or COMMA.. 
                    if (wholeWords && !(WHITE_SPACE.Contains(afterPlaceholder) || CLOSED_PAREN.Equals(afterPlaceholder) || COMMA.Equals(afterPlaceholder)))
                    {
        ...
        public static string ToString(object[] array)
        {
            ...
            {
                buf.Append(NullSafeToString(array[i])).Append(COMMA_SPACE);
            }
        ...
        public static int CountUnquoted(string str, char character)
        {
            ...
            for (int indx = 0; indx < stringLength; indx++)
            {
                if (inQuote)
                {
                    if (SINGLE_QUOTE == chars[indx])
                    {
                        inQuote = false;
                    }
                }
                else if (SINGLE_QUOTE == chars[indx])
                {
                    inQuote = true;
                }
        ...
        public static string Qualify(string prefix, string name)
        {
            ...
            if (!string.IsNullOrEmpty(prefix) && first != SINGLE_QUOTE && !char.IsDigit(first))
            {
        ...
        {
            return GenerateAliasRoot(description) + UNDER_SCORE;
        }
    ...
    public class TypeNameParser
    {
        ...
        public AssemblyQualifiedTypeName ParseTypeName(string typeName)
        {
            ...
            string type = WhiteSpaces.Replace(typeName, " ");
            ...
            {
                bool isArrayType = type.EndsWith("[]");
                ...
                List<AssemblyQualifiedTypeName> genericTypeArgs = new List<AssemblyQualifiedTypeName>();
                ...
                {
                    AssemblyQualifiedTypeName typeArgument = ParseTypeName(item);
                                                         ...
                                                         AssemblyQualifiedTypeName[] typeArguments)
        {
            ...
            string baseType = qualifiedName.Type;
            StringBuilder sb = new StringBuilder(typeArguments.Length * 200);
            sb.Append(baseType);
            sb.Append('[');
